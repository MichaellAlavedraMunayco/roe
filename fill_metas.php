<?php
  ...
  while ($rowind = mysqli_fetch_array($resultind)) {
      ...
      echo '<form class="border-bottom py-3" action="edit_meta.php?id_concepto=' . $id_concepto . '&nombre_concepto='.$nombre_concepto.'" method="post">';
      ...
      while ($rowper = mysqli_fetch_array($resultper)) {
          echo '<div class="col"><input class="form-control" type="number" name="' . $rowper['id_periodo'] . '" value="' . $rowper['valor'] . '" required></div>';
      }
      if (mysqli_num_rows($resultper) == 0) {
        ...
        if (mysqli_num_rows($resultper2) > 0) {
            ...
            $count = $rowper2[0];
            for ($i = 1; $i <= $count; $i++) {
                echo '<div class="col"><input class="form-control" type="number" name="' . $i . '" required></div>';
            }
        }
      }
      echo '<div class="col"><input type="submit" class="btn btn-success btn-block" name="edit_meta" value="Act"></div>';
      echo '</div>';
      echo '</form>';
  }
?>
